...
.PHONY: help makebuilddir build clean test coverage coverage-html fmt fmt-check vet lint staticcheck godoc
...
coverage-cobertura: $(BUILD_DIR)/coverage/coverage.txt
	$(WHICH) gocover-cobertura || ($(GO) get github.com/boumenot/gocover-cobertura && $(GO) install github.com/boumenot/gocover-cobertura && $(GO) mod tidy)
	$(GOBIN)/gocover-cobertura < $(BUILD_DIR)/coverage/coverage.txt > $(BUILD_DIR)/coverage/cobertura.xml
...
lint: makebuilddir
	$(WHICH) golint || ($(GO) get golang.org/x/lint/golint && $(GO) install golang.org/x/lint/golint && $(GO) mod tidy)
	$(GO) list ./... | grep -v /vendor/ | grep -v docs | xargs -L1 $(GOBIN)/golint -set_exit_status

staticcheck: makebuilddir
	$(WHICH) staticcheck || ($(GO) get honnef.co/go/tools/cmd/staticcheck && $(GO) install honnef.co/go/tools/cmd/staticcheck && $(GO) mod tidy)
	rm -rf $(BUILD_DIR)/staticcheck
	mkdir -p $(BUILD_DIR)/staticcheck
	ln -s "$(GO)" $(BUILD_DIR)/staticcheck/go
	PATH="$(PWD)/$(BUILD_DIR)/staticcheck:$(PATH)" $(GOBIN)/staticcheck ./...
	rm -rf $(BUILD_DIR)/staticcheck
...
godoc: makebuilddir
	$(WHICH) godoc || ($(GO) get -v golang.org/x/tools/cmd/godoc && $(GO) install -v golang.org/x/tools/cmd/godoc && $(GO) mod tidy)
	$(GOBIN)/godoc -http=:58080 -index

download: makebuilddir
